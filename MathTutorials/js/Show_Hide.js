...
var UseCookies = true;         // Use a cookie to keep track of which show/hide  
	...
	for (i=0; i < oDivs.length; i++ ) {
		...
		if (div.id.indexOf("Block_") != -1) {
			...
			if (!UseCookies) {
				...
				Set_Cookie(div.id,'0','','index.html','','');
				DefDisplay = '0';
			}
...
function ShowHide(type,id, imgtype) {
	...
	if (UseCookies) {
		Set_Cookie(objDiv.id,DefDisplay,'','index.html','','');	
	}
	...
	if (CurLoc.indexOf("Solutions/index-2.html") != -1 && parent.location.href != CurLoc) {
...
function ShowHideAll(type, action) {
	...
	for (i=0; i < oDivs.length; i++ ) {
		...
		if (div.id.indexOf("Block_") != -1) {
			...
			// Find the previous display option and set the cookie if needed.
			Set_Cookie(div.id,action,'','index.html','','');
		...
		if (isBalloon) {
		    if (didAdjTop) {
		        ...
		        if (didAdjSide) {
		            ...
		            DHTMLAPI.setBGImage("BalTop_"+id, "Images/Balloon_Top_Left-2.gif", "", "left bottom");
		        }
		        ...
		        if (didAdjSide) {
		            ...
		            DHTMLAPI.setBGImage("BalBot_"+id, "Images/Balloon_Bottom_Left-2.gif", "", "left top");
		        } 
		    }
		}
